...
void Flag<T>::set_value_as_str(const std::string &value_as_str) {
  ...
  if constexpr (std::is_same_v<T, std::string>) {
    ...
    parsed = value_as_str.empty()
                 ? (value_ = true, true)
                 : absl::SimpleAtob(value_as_str, &value_);
  } else if constexpr (std::is_integral_v<T>) {
    parsed = absl::SimpleAtoi(value_as_str, &value_);
  } else if constexpr (std::is_floating_point_v<T>) {
    double value = 0.0;
    parsed = absl::SimpleAtod(value_as_str, &value);
    if (parsed) value_ = static_cast<T>(value);
  }
  if (!parsed) {
    std::cerr << "Invalid value for flag --" << func_->name << ": "
              << value_as_str << std::endl;
    std::exit(1);
  }
}
